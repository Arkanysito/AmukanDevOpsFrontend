...
export default function CrearServicio() {
  ...
  const { serviceType } = useParams(); // "accommodation" o "activity"
  ...
  // --- handleGuardar (Lógica de accesibilidad ELIMINADA de newPlacePayload) ---
  const handleGuardar = async () => {
    ...
    try {
      ...
      if (isCreatingNewPlace) {
        // 1. Construir y guardar el NUEVO LUGAR
        const newPlacePayload = {
          ...
          average_price: parseFloat(precio) || 0 // Re-usamos el precio del servicio
        };
      ...
      console.log("Enviando a createService:", serviceData);
      result = await serviceService.createService(serviceData);

      Swal.fire({
        icon: "success",
        title: `¡${tipoCreado} creado!`,
  ...
  // Renderizar campos específicos según tipo de servicio
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            <div>
              ...
              <select 
                ...
                className={errores.accommodationType ? inputErrorStyle : inputStyle}
              >
                <option value="">Selecciona tipo</option>
                {choices.accommodationType && choices.accommodationType.map((tipo) => (
    ...
    return (
      <div className="flex flex-col gap-4">
        <label className="block text-sm font-medium text-gray-700">
          {loading ? "Buscando zona..." : "Haz clic en el mapa para ubicar el lugar"}
        </label>
      ...
      <div className="flex flex-col gap-4 mr-0 md:mr-10 ml-0 md:ml-10 text-left ">
        ...
        {seccionAbierta.informacion && (
          <div className="flex flex-col gap-4 px-6 pb-6">
            <div>
              ...
              <input type="text" value={nombre} onChange={e => setNombre(e.target.value)} className={errores.nombre ? inputErrorStyle : inputStyle} placeholder={serviceType === 'accommodation' ? "Ej: Cabañas del Bosque" : "Ej: City Tour"} />
            </div>
            ...
            <div>
              ...
              <div className="bg-gray-100 px-4 py-2 rounded-md text-gray-800 font-semibold">
                {nombresTipo[serviceType] || "Servicio"}
              </div>
            </div>
        ...
        {seccionAbierta.general && (
          <div className="flex flex-col gap-4 px-6 pb-6">
            ...
            {seccionAbierta.descripcion && (
              <div className="flex flex-col gap-4 px-6 pb-6">
                <div>
                  ...
                  <textarea value={descripcion} onChange={e=>setDescripcion(e.target.value)} className={errores.descripcion?textareaErrorStyle:textareaStyle} rows="3" placeholder="Describe el servicio ofrecido..."></textarea>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Políticas</label>
                  <textarea value={policies} onChange={e=>setPolicies(e.target.value)} className={errores.policies?textareaErrorStyle:textareaStyle} rows="2" placeholder="Políticas de cancelación, horarios, etc"></textarea>
                </div>
                <div>
              ...
              <div className="flex flex-col gap-4 px-6 pb-6">
                { !isCreatingNewPlace ? (
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onChange={(opcionSeleccionada) => {
                          setPlaceId(opcionSeleccionada ? opcionSeleccionada.value : "");
                        }}
                  ...
                  <>
                    ...
                    <button 
                      type="button"
                      onClick={() => setIsCreatingNewPlace(false)}
              ...
              <div className="flex flex-col gap-4 px-6 pb-6">
                ...
                <div>
                  ...
                  <select value={moneda} onChange={e=>setMoneda(e.target.value)} className={errores.moneda?inputErrorStyle:inputStyle}>
                    {choices.currency && choices.currency.map((moneda) => (
            ...
            {/* --- SECCIÓN EXTRAS (Accesibilidad ELIMINADA) --- */}
  ...
  return (
    <div className="flex flex-col bg-white min-h-screen w-full overflow-y-auto">
      ...
      <h1 className="text-2xl font-bold text-gray-800 px-6 pt-4">
        Crea tu nuev{serviceType === 'accommodation' ? 'o' : 'a'} {nombresTipo[serviceType] || "publicación"}
      </h1>
